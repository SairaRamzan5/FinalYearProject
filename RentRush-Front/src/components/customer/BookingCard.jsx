...
function BookingCard({
  ...
  // Complaint Functions - UPDATED: Proof is now optional
  const validateComplaintField = (name, value) => {
    let error = "";
    switch (name) {
      case "compl_Against":
  ...
  const clearProofFile = () => {
    setComplaintForm(prev => ({ ...prev, proof: null }));
  ...
  // UPDATED: Submit complaint with optional proof
  const handleSubmitComplaint = async (e) => {
    e.preventDefault();
    setIsSubmitting(true);

    // ✅ UPDATED: Only validate required fields (proof is optional)
    const requiredFields = ["compl_Against", "description"];
    let isValid = true;
    ...
    try {
      ...
      // ✅ UPDATED: Only append proof if provided (optional)
      if (complaintForm.proof) {
        formDataToSend.append("proof", complaintForm.proof);
      }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md overflow-hidden flex flex-col w-full max-w-[380px] h-full transition duration-300 hover:shadow-lg border border-gray-200 relative">
      ...
      {/* UPDATED: Complaint Modal with Optional Proof */}
      {showComplaintModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            {/* Modal Header */}
              ...
              <form onSubmit={handleSubmitComplaint} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    {/* Clear file button - only show when file is selected */}
                    {complaintForm.proof && (
                      <button
                        type="button"
                        onClick={clearProofFile}
                        className="px-3 py-2  text-white rounded-lg  text-md whitespace-nowrap"
                      >
                        Clear
                      </button>
                    )}
                  ...
                  {complaintForm.proof && (
                    <p className="text-xs text-green-600 mt-1">
                      Selected: {complaintForm.proof.name}
                    </p>
                  )}
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Supported formats: Images, PDF, Word documents (Optional)
                  </p>
                </div>

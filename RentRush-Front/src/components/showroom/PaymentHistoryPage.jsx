...
const Base_Url = import.meta.env.VITE_API_URL || "http://localhost:3000"; // ✅ Port correct karo
...
const PaymentHistoryPage = () => {
  ...
  const fetchPaymentHistory = async () => {
    try {
      setLoading(true);
      console.log("🔄 Fetching SHOWROOM payment history...");
      
      // ✅ CORRECT API ENDPOINT
      const response = await axios.get(
        `${Base_Url}/api/payments/showroom/payment-history`,
        { withCredentials: true }
      );
      
      console.log("✅ Showroom payment history response:", response.data);
  ...
  // ✅ HELPER FUNCTION TO GET CAR DETAILS
  const getCarDetails = (payment) => {
    if (payment.carId && typeof payment.carId === 'object') {
      return {
        brand: payment.carId.carBrand || 'N/A',
        model: payment.carId.carModel || 'N/A',
        number: payment.carId.plateNumber || payment.carId.carNumber || 'N/A'
      };
    }
  ...
  const getCustomerDetails = (payment) => {
    if (payment.customerId && typeof payment.customerId === 'object') {
      return {
        name: payment.customerId.ownerName || 'N/A',
        phone: payment.customerId.contactNumber || payment.customerId.phone || 'N/A'
      };
    }

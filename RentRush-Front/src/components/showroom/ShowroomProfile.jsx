...
const ShowroomProfile = () => {
  ...
  const fetchShowroomProfile = async () => {
    try {
      ...
      // Update session storage immediately when profile loads
      sessionStorage.setItem("showroomName", updatedProfileData.showroomName);
      sessionStorage.setItem("name", updatedProfileData.ownerName);
      sessionStorage.setItem("profilePicture", updatedProfileData.profileImage);
  ...
  const handleSave = async () => {
    try {
      ...
      // Refresh profile data to get any server-side changes
      await fetchShowroomProfile();
      
      // Dispatch event to notify navbar and other components
      window.dispatchEvent(new CustomEvent('showroomProfileUpdated', {
        detail: {
          showroomName: profileData.showroomName,
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      sessionStorage.setItem("profilePicture", newProfileImage);
      ...
      console.log("Profile image updated event dispatched");
      toast.success("Profile image updated successfully");

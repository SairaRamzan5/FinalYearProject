...
const CarMaintenancePage = () => {
  ...
  const fetchVehicles = async () => {
    try {
      ...
      // ✅ Cars ko "In Maintenance" status ke sath set karen
      const carsWithMaintenanceStatus = response.data.map(car => ({
        ...car,
        availability: "In Maintenance" // ✅ Status update karo
      }));
      
      setCars(carsWithMaintenanceStatus);
    } catch (err) {
      console.log(err);
      Toast(err.data || err.message || "Something went wrong", "error");
    }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-6 p-3 justify-items-center">
          {cars?.map((car) => (
            <div
              ...
              className="cursor-pointer"
            >
              {/* ✅ CarCard ko modified car data pass karen with "In Maintenance" status */}
              <CarCard car={{...car, availability: "In Maintenance"}} />
            </div>
          ))}
        </div>

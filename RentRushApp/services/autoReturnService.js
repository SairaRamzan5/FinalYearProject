...
// autoReturnService.js - FIXED VERSION (Grace period only for charges)
import cron from "node-cron";
import Booking from "../Model/bookingModel.js";
import Car from "../Model/Car.js";
import moment from "moment";
...
export const startAutoReturnService = () => {
  console.log("🚗 Auto-return service started (Grace period only for charges)...");
...
// Step 1: Auto initiate returns - FIXED (Immediate return, grace period only for charges)
const autoInitiateReturns = async () => {
  try {
    const currentDateTime = moment();
    console.log(`🔍 Auto-initiate check at: ${currentDateTime.format()}`);
    ...
    for (const booking of bookingsToInitiate) {
      try {
        ...
        console.log(`   End: ${rentalEndMoment.format()}`);
        console.log(`   Now: ${currentDateTime.format()}`);
        console.log(`   Is Past End Time: ${currentDateTime.isAfter(rentalEndMoment)}`);

        // ✅ IMMEDIATE RETURN WHEN RENTAL PERIOD ENDS (No grace period for car status)
        if (currentDateTime.isAfter(rentalEndMoment)) {
          // ✅ AUTO INITIATE RETURN IMMEDIATELY
          booking.status = "return initiated";
          booking.autoReturnInitiated = true;
          booking.returnInitiatedAt = new Date();

          // ✅ CALCULATE OVERDUE CHARGES WITH 1-HOUR GRACE PERIOD
          const gracePeriodEndMoment = rentalEndMoment.clone().add(1, 'hour');
          let overdueHours = 0;
          let overdueCharge = 0;

          // Only charge if current time is AFTER grace period
          if (currentDateTime.isAfter(gracePeriodEndMoment)) {
            const diffInMs = currentDateTime.diff(gracePeriodEndMoment);
            const diffInHours = diffInMs / (1000 * 60 * 60);
          ...
          // Update car status to Pending Return IMMEDIATELY
          const car = await Car.findById(booking.carId._id);
          if (car) {
            car.availability = "Pending Return";
            await car.save();
          ...
          console.log(`✅ Booking ${booking._id} auto-initiated return`);
          console.log(`   Car moved to Pending Return immediately`);
          console.log(`   Overdue: ${overdueHours} hours, Charge: ₹${overdueCharge}`);
          console.log(`   Grace period for charges ended at: ${gracePeriodEndMoment.format()}`);
        }
      } catch (bookingError) {
        console.error(`❌ Error auto-initiating return for booking ${booking._id}:`, bookingError);
      }
